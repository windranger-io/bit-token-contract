...
describe("BitDAO token contract", async () => {
  ...
  it("minting BIT not allowed", async () => {
    expect(dao.transferFrom(ZERO_ADDRESS, admin, 5)).to.be.revertedWith("ERC20: transfer from the zero address");
  });

  it("burning BIT not allowed", async () => {
    expect(dao.transferFrom(admin, ZERO_ADDRESS, 5)).to.be.revertedWith("ERC20: transfer to the zero address");
  });
